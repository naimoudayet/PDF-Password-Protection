...
All notable changes to **PDF Password Protection** for Odoo 18.0 are documented here.

This file follows the [Keep a Changelog](https://keepachangelog.com/en/1.1.0/) format.
Versions use Odoo's `<odoo_version>.<module_major>.<module_minor>.<module_patch>` scheme.

## [18.0.2.0.0] - 2026-09-01
...
- **Printing several customers at once no longer leaks one customer's document to another.** Odoo merges a multi-record print into a single PDF carrying one password; with a dynamic source that password came from the first record, so the file opened for one recipient and exposed every other recipient's document inside it. Such a batch is now produced **without** protection and a note is added to each record. An earlier build of this release blocked the print outright, which was the wrong call - staff print batches all day and interrupting that is worse than handing back an honestly unlocked file.
...
- Translations regenerated for all 9 languages via `odoo --i18n-export`. The `model:ir.module.module,summary|shortdesc|description` entries carried by earlier `.po` files were dropped: `module_meta_information` is no longer a translatable target, so those entries translated nothing.

- **Protection now follows delivery, not storage - including for invoices.** The companion module encrypts the invoice as it enters the customer's email and as the portal serves it, rather than encrypting the file at rest. Core builds the mail's attachments as byte copies, so the record keeps a readable original: your own people open a sent invoice without hunting down the customer's VAT number, while what actually left the building is locked. This replaces the earlier at-rest approach, which protected the same documents but made staff type a password to look at their own records.

- **The email can explain the password.** A protected invoice used to arrive with no explanation, leaving the recipient with a file that would not open and an email that said nothing about it. The Send dialog now carries an option, on by default and only shown when that report has protection enabled. The wording is a translatable Html field on the report, edited in the UI, so nobody has to touch the module to reword it or phrase it per language - and because the module only ever inserts your text, it can never write a password into the message carrying the document. It is added in `_get_mail_params`, after we know the document really came out encrypted, so a PDF/A skip or an unresolved password can never leave a false promise behind.
...
- Module version bumped from `18.0.1.1.0` to `18.0.1.2.0` (semver minor for new feature + translation fix).
