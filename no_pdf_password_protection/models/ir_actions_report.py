...
class IrActionsReport(models.Model):
    ...
    def _pdf_encryption_suppressed(self):
        ...
        if "snailmail_layout" in self.env.context:
            ...
            return True
        if self.env.context.get("no_pdf_defer_encryption"):
            # The accounting send pipeline post-processes the freshly rendered
            # invoice - account_edi_ubl_cii reopens it to embed the e-invoicing
            # XML - and cannot read an encrypted file. The companion module sets
            # this while that render runs and encrypts afterwards instead, as
            # the document enters the customer's mail.
            _logger.debug(
                "Deferring PDF password protection: the caller post-processes "
                "this render and encrypts the outgoing copy itself."
            )
            return True
        return False
